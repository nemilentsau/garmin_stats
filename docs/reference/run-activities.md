...
- `backend/app/domains/garmin_sync/infra/activity_ingest.py::ingest_running_activities(activities_dir, force=False)` — the single entry point. Fingerprint-gated and idempotent:
  1. Computes a versioned SHA-256 signature for each discovered primary running FIT plus its optional sidecar, then fingerprints that source set. Stored/compared against `ingest_meta` key **`activities_fingerprint`**. Parser-owned identity changes can bump the source-signature version to force a one-time migration re-ingest.
  2. If the fingerprint is unchanged and `force=False`, returns immediately (`skipped=True`) — no file is decoded.
  3. Otherwise, discovers running files and diffs their signatures against `running_activity_sources`; new and changed sources are parsed, while sources no longer present on disk have their session, lap, series, and source rows deleted. A per-file parse failure is logged and counted in `files_failed`; it does not abort the rest of the batch.
  4. Writes/replaces rows in `running_activity_sessions` (one per run), `running_activity_laps` (one per lap, delete-then-insert per session), and `running_activity_series` (one JSON-blob row per session). The fingerprint advances only when the batch has no parse failures, so a failed FIT is retried even when the tree itself is unchanged. The read cache (`app.infra.cache`) is invalidated when sessions are written or removed.
...
- **Adding a parser field requires an activity re-ingest.** Unchanged per-source signatures skip decoding, so new parser fields reach already-stored rows only via a full rebuild (`cd backend && uv run python ../scripts/reingest_activities.py`) or an intentional source-signature version bump for a required migration (see `data-and-ingest.md` Commands).
...
**`running.v3` discriminator.** Association only ever evaluates for cards from the `running.v3` bundle (`training/application/compile.py::is_running_bundle`, imported by both `read_models.py` and `validation.py` — there is no separate "own check" in `validation.py`, it calls the same helper) — every other card, including `support.v3`'s `sup.daily` (which also prescribes `SegmentPrescription`), gets `run_candidates=[]`/`associated_activity=None` unconditionally.
...
Zone display is not a known gap: `garmin_analytics/domain/run_display.py::zone_display_rows` caps at 5 numbered zones and renders the top zone open-ended (`"≥{lower}"`) rather than showing a raw upper-boundary sentinel — see [`coach.md`](coach.md) for the shared projection both the run UI and Coach consume.

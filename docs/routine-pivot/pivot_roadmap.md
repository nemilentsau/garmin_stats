...
- **Activity FIT files download** from Garmin Connect on every sync into `data/garmin_activities/`. **Running now parses** — session/lap/series ingest, fingerprint-gated on sync + startup, served at `/api/activities/runs*` and displayed at `/runs` + `/runs/[id]` (`docs/reference/run-activities.md`). Strength and breathing files still just download.
- **The app computes no training signals yet.** It collects; it does not yet estimate. That is the next step.

## Next steps — in this order

1. **Parse the workout files** (running + strength, session grain first). **Running half shipped 2026-07-11**: parse + `/runs` UI + `hr_source`/strap-validity detection (`docs/reference/run-activities.md`). **Strength half remains** — spec: `docs/future/STRENGTH_ACTIVITY_SCHEMA.md`. Association between a prescribed card and its actual activity is not built for either sport yet.

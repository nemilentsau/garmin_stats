...
**Validator** (`application/validation.py`): ports `block0/linter.py` L1–L12 rule-for-rule. Soft-spot dispositions (calibrated so the shipped artifacts still lint 0/0 — they are the adopted truth): **L7 hardened** — a state component counts as covered only if a `cap.*` capture field is reachable through the estimator DAG transitively (S4 is covered via `tonnage.upper.7d` ← `est.upper_tonnage` ← `cap.set_log.*`), strictly stronger than the linter's any-ambient-input pass; **L11 hardened** — week-1 novelty computed (distinct overload adaptation kinds in week 1) instead of the hardcoded `novel = 3`; **L9 kept as-ported** (unramped-novel-overload check covers `tendon_stiffness` only) — a general novelty definition needs capture history that doesn't exist yet; deferred to a later Phase 1 slice and noted in the roadmap. Errors block activation; warnings require an explicit ack recorded with the import.

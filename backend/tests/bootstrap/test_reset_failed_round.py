...
def test_reset_rejects_missing_database_without_creating_it_or_touching_coach(
    tmp_path: Path,
):
    db_path = tmp_path / "missing.sqlite"
    coach_dir = tmp_path / "coach"
    coach_dir.mkdir()
    marker = coach_dir / "workspace.md"
...
def test_reset_rejects_unrelated_database_without_touching_coach(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="not a Garmin Stats database"):
        reset_failed_round(
            db_path=db_path,
            wellness_dir=tmp_path / "wellness",
            activities_dir=tmp_path / "activities",

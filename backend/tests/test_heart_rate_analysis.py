...
class TestCircadianProfile:
    def test_always_returns_24_points(self):
        result = _compute_circadian_profile([])
    ...
    def test_averages_by_hour(self):
        ...
        result = _compute_circadian_profile([w])
        hour_8 = result[8]
...
class TestSleepingHRTrend:
    def test_only_sleep_stage_readings_included(self):
        ...
        result = _compute_sleeping_hr_trend([w_prev, w_curr], [sleep])
        assert len(result) == 1
    ...
    def test_dates_without_sleep_data_skipped(self):
        ...
        result = _compute_sleeping_hr_trend([w], [])
        assert result == []
    ...
    def test_zero_hr_excluded(self):
        ...
        result = _compute_sleeping_hr_trend([w], [sleep])
        assert len(result) == 1
...
class TestRestingHRTrend:
    def test_first_day_ma_equals_self(self):
        ...
        result = _compute_resting_hr_trend(metrics)
        assert len(result) == 1
...
class TestHRDistribution:
    def test_5_bpm_bins(self):
        ...
        result = _compute_hr_distribution(readings)
        assert len(result) == 3
    ...
    def test_empty_readings(self):
        assert _compute_hr_distribution([]) == []
...
class TestWeeklyBoxplots:
    def test_iso_week_grouping(self):
        ...
        result = _compute_weekly_boxplots(metrics)
        assert len(result) == 2

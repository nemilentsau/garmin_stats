...
@pytest.mark.parametrize(
    "payload",
    [
        {"card_type": "running_workout", "workout_type": "easy"},
        {"card_type": "strength_session"},
    ],
)
def test_retired_v2_training_payloads_are_rejected(payload: dict[str, str]):
    with pytest.raises(ValidationError):
        PAYLOAD_ADAPTER.validate_python(payload)
...
def test_checklist_variant_fields_default_empty():
    checklist = ChecklistPayload(card_type="checklist")
    assert checklist.variant_options == []
    assert checklist.selection_rule is None


def test_checklist_variant_fields_are_settable():
...
@pytest.mark.parametrize("card_type", ["running_workout", "strength_session"])
def test_retired_v2_training_actuals_are_rejected(card_type: str):
    with pytest.raises(ValidationError):
        ACTUAL_ADAPTER.validate_python({"card_type": card_type})

"""Tests for deterministic assistant intent classification."""

from app.domains.assistant.application.intent_routing import route_user_query


def test_intent_routing_classifies_experiment_review_questions() -> None:
...
def test_intent_routing_classifies_recovery_briefing_questions() -> None:
    decision = route_user_query("Give me a quick recovery briefing for today")

    assert decision.intent == "recovery_briefing"


def test_intent_routing_classifies_routine_adherence_questions() -> None:
    decision = route_user_query("Did I stick to my routine this week?")

    assert decision.intent == "routine_adherence"


def test_intent_routing_classifies_trial_questions_as_experiment_review() -> None:
    decision = route_user_query("How is the meditation trial going?")

    assert decision.intent == "experiment_review"


def test_intent_routing_classifies_plural_experiment_scan_questions() -> None:
...
def test_intent_routing_marks_shipped_experiment_scan_prompt_as_scan_context() -> None:
...
def test_intent_routing_classifies_weekly_review_questions_as_open_ended_coaching() -> None:
    query = (
...
def test_intent_routing_marks_explicit_recall_language_signal() -> None:
    decision = route_user_query(
...
def test_intent_routing_does_not_treat_time_window_comparisons_as_thread_recall() -> None:

...
## Confidence (set in 05-findings.md, may only be downgraded in 06-review.md)

- `tentative` — single recipe, no replication, exploratory. Stays in run folder. **A single eye-catching result from a many-comparison sweep is tentative, not more.**
- `provisional` — multiple recipes converge, or a hypothesis-test passed its pre-registered rule. Stays pending review.
- `confident` — provisional **plus** a passing extended-run review. Only `confident` findings promote.

## Quality gates (all must pass before promotion)

1. **Question** — concrete question, explicit date range + metrics, run type declared, pre-registration present for hypothesis tests.
...
6. **Findings** — only `confident` promotes; promotion records snapshot date, date range, sample size, caveats; any resolved open question moves in the same patch.

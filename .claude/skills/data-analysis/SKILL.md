...
1. **Aggregation layer** — what statistics to compute from raw data, how to compute them, what the API exposes (`domains/garmin_health/domain/daily_metrics/`, `domains/garmin_analytics/domain/aggregates/`, Garmin health/analytics contracts)
...
This section applies whenever you write or modify code that computes statistics from raw readings — `domains/garmin_health/domain/daily_metrics/`, `domains/garmin_analytics/domain/aggregates/`, Garmin health/analytics aggregate contracts, or any new summary computation.
...
When adding a new numeric metric to the persisted daily metric mart:
...
This script loads the same data the frontend uses (via parser + Garmin health daily metrics), generates matplotlib charts matching the frontend layout, and saves them as PNGs. Read the PNGs with multimodal capabilities to check for issues.
...
- **Full trace (all 5 steps)**: Adding a new metric end-to-end (parser → Garmin health/analytics metrics → frontend)

...
- Experiment exposures are explicit `experiment_id + date` records, not card-level records. Record whether the experiment protocol's prescribed daily dose/frequency was satisfied across every required session or component for that date.

<script lang="ts">
	 ...
	 * pace distribution — a display mapping of already-computed values (same class as the
	 * time-in-zone bar widths on this page), never a statistical transform. Quantile (equal-
	 * population) edges are used rather than equal-width min/max bins because urban runs carry
	 * extreme stop/light outliers (this route: 6.6-27.1 min/mi) that would otherwise pack ~99%
	 * of segments into a single bin and make the coloring invisible — quantiles keep the color
	 * spread meaningful across the run's actual pace variation. Segments with no pace on either
	 * endpoint draw neutral gray rather than being dropped, so GPS-but-no-pace stretches (e.g.
	 * paused/stopped) still read as part of the route.
	 *
	...
	const PACE_BIN_COLORS = ['#E85D4A', '#DB8248', '#D4944C', '#7FA3C9', '#4FA8DE'] as const;
	...
	function buildPoints(): RoutePoint[] {
		const points: RoutePoint[] = [];
		for (let i = 0; i < lat.length; i++) {
			const la = lat[i];
			const lo = lon[i];
			if (la == null || lo == null) continue;
			points.push({ lat: la, lon: lo, pace: pace[i] ?? null });
		}
		return points;
	}
	...
	onMount(() => {
		...
		(async () => {
			...
			if (points.length < 2) return; // no usable GPS trace — leave the panel empty
